...
export default function SystemPage() {
  ...
  const [adapterConfigModal, setAdapterConfigModal] = useState<{
    type: string;
    adapterId?: string;
    isEdit?: boolean;
  } | null>(null);
  ...
  const runtimeStatus = runtimeState
    ? {
        ...
        cognitive_state:
          (runtimeState.cognitive_state !== "UNKNOWN"
            ? runtimeState.cognitive_state
            : healthData?.cognitive_state?.toUpperCase()) || "WORK",
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-amber-800 mt-1">
              The adapter configuration and system monitoring features are currently under active
              development. Some functionality may be incomplete or change in future updates.
            </p>
          </div>
        </div>
      </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              <div className="bg-gray-50 px-4 py-5 sm:p-6 rounded-lg border-2 border-gray-200">
                ...
                <dd className="mt-2 flex items-center">
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-lg font-semibold bg-${getHealthColor(health.status)}-100 text-${getHealthColor(health.status)}-800`}
                  >
              ...
              <div className="bg-gray-50 px-4 py-5 sm:p-6 rounded-lg border-2 border-gray-200">
                ...
                <dd className="mt-2 text-2xl font-semibold text-gray-900">
                  {resources?.current_usage?.memory_mb
                    ? `${resources.current_usage.memory_mb.toFixed(1)} MB`
                    : "N/A"}
                </dd>
              </div>
              ...
              <div className="bg-gray-50 px-4 py-5 sm:p-6 rounded-lg border-2 border-gray-200">
                ...
                <dd className="mt-2 text-2xl font-semibold text-gray-900">
                  {resources?.current_usage?.cpu_percent
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-lg font-bold ${resources?.current_usage?.cpu_percent > 80 ? "text-red-600" : resources?.current_usage?.cpu_percent > 60 ? "text-yellow-600" : "text-green-600"}`}
                  >
                  ...
                  <div className="overflow-hidden h-4 text-xs flex rounded-full bg-gray-200">
                    <div
                      ...
                      className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-300 ${
                        resources?.current_usage?.cpu_percent > 80
                          ? "bg-red-500"
                          : resources?.current_usage?.cpu_percent > 60
                            ? "bg-yellow-500"
                            : "bg-blue-500"
                      }`}
                    />
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-lg font-bold ${resources?.current_usage?.memory_percent > 80 ? "text-red-600" : resources?.current_usage?.memory_percent > 60 ? "text-yellow-600" : "text-green-600"}`}
                  >
                    {resources?.current_usage?.memory_mb
                      ? resources.current_usage.memory_mb.toFixed(1)
                      : 0}{" "}
                    MB
                  </span>
                </div>
                <div className="relative">
                  <div className="overflow-hidden h-4 text-xs flex rounded-full bg-gray-200">
                    <div
                      ...
                      className={`shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center transition-all duration-300 ${
                        resources?.current_usage?.memory_percent > 80
                          ? "bg-red-500"
                          : resources?.current_usage?.memory_percent > 60
                            ? "bg-yellow-500"
                            : "bg-green-500"
                      }`}
                    />
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-lg font-bold text-green-600">
                    {resources?.current_usage?.disk_used_mb
                      ? `${(resources.current_usage.disk_used_mb / 1024).toFixed(1)} GB`
                      : "N/A"}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-2">
                <div>
                  <p className="text-2xl font-bold text-green-700">
                    {telemetryOverview?.carbon_last_hour_grams
                      ? (telemetryOverview.carbon_last_hour_grams / 1000).toFixed(3)
                      : "0.000"}{" "}
                    kg
                  </p>
              ...
              <div className="space-y-2">
                <div>
                  <p className="text-2xl font-bold text-blue-700">
                    {telemetryOverview?.energy_last_hour_kwh
                      ? telemetryOverview.energy_last_hour_kwh.toFixed(4)
                      : "0.0000"}{" "}
                    kWh
                  </p>
                  <p className="text-xs text-gray-600">Last hour total</p>
                ...
                <div>
                  <p className="text-2xl font-bold text-purple-700">
                    ...
                    {telemetryOverview?.cost_last_hour_cents
                      ? (telemetryOverview.cost_last_hour_cents / 100).toFixed(2)
                      : "0.00"}
                  </p>
                  <p className="text-xs text-gray-600">Last hour total</p>
                </div>
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
              <div className="bg-gray-50 rounded p-3">
                ...
                <p className="text-lg font-semibold text-gray-900">
                  {telemetryOverview?.tokens_24h
                    ? telemetryOverview.tokens_24h.toLocaleString()
                    : "0"}
                </p>
              </div>
              <div className="bg-gray-50 rounded p-3">
        ...
        <div className="px-4 py-5 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-lg font-medium text-gray-900">
              Services Health{services?.services ? ` (${services.services.length} Services)` : ""}
            </h3>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-500 mt-1">{service.service_type}</p>
                    </div>
                    <StatusDot
                      status={
                        service.healthy === true
          ...
          <div className="px-4 py-5 sm:p-6">
            ...
            <div className="mt-4 p-4 bg-blue-50 rounded-lg">
              <p className="text-sm text-blue-800">
                <strong>Note:</strong> The CIRIS system has one main processor that cycles through
                cognitive states (WAKEUP, WORK, PLAY, DREAM, SOLITUDE, SHUTDOWN). Pausing affects
                the entire processor, not individual states.
              </p>
            </div>
          </div>
        </div>
                ...
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-500 mt-1">ID: {channel.channel_id}</p>
                      <p className="text-xs text-gray-500">Type: {channel.channel_type}</p>
                    </div>
                    <StatusDot
                    ...
                    <tr key={adapter.adapter_id}>
                      ...
                      <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                        <div className="flex flex-col">
                          <span className="font-medium">
                            {channels?.filter((ch: any) => ch.channel_type === adapter.adapter_type)
                              .length || 0}{" "}
                            active
                          </span>
                      ...
                      <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                        <div className="flex items-center justify-end space-x-3">
                          ...
                          <button
                            onClick={() =>
                              setConfirmDialog({ type: "reloadAdapter", name: adapter.adapter_id })
                            }
      ...
      {/* Simple Test Modal */}
      {confirmDialog &&
        confirmDialog.type === "registerAdapter" &&
        confirmDialog.name === "test" && (
          <div
            style={{
              position: "fixed",
              top: 0,
        ...
        >
          ...
          >
            ...
            <p style={{ marginBottom: "20px", color: "#666" }}>
              {confirmDialog.type === "pauseRuntime" &&
                "Are you sure you want to pause the runtime? This will temporarily stop all message processing."}
              {confirmDialog.type === "resumeRuntime" &&
                "Are you sure you want to resume the runtime? Message processing will continue."}
              {confirmDialog.type === "reloadAdapter" &&
                `Are you sure you want to reload the ${confirmDialog.name} adapter?`}
              {confirmDialog.type === "unregisterAdapter" &&
                `Are you sure you want to remove the ${confirmDialog.name} adapter? This cannot be undone.`}
              {confirmDialog.type === "registerAdapter" &&
                `Are you sure you want to register a new ${confirmDialog.name} adapter?`}
            </p>
        ...
        <AdapterConfigModal
          ...
          onSubmit={(adapterType, config) => {
            if (adapterConfigModal.isEdit && adapterConfigModal.adapterId) {
              ...
              cirisClient.config
                .updateConfigByKey(configKey, config)
                .then(() => {
