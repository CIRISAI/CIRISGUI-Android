...
export class Transport {
  ...
  private async handleResponse<T>(
    ...
    try {
      ...
      // Identify if this is a manager endpoint (expected to fail on local/mobile)
      const isManagerEndpoint = response.url.includes("/manager/");
      const logPrefix = isManagerEndpoint ? "[CIRIS SDK] Manager API" : "[CIRIS SDK]";

      // Log the parsing error with context (stringify for Android logcat compatibility)
      console.error(
        `${logPrefix} Failed to parse JSON response:`,
        JSON.stringify({
          status: response.status,
          statusText: response.statusText,
          url: response.url,
  ...
  private isSuccessResponse(data: any): data is SuccessResponse {
    return typeof data === "object" && data !== null && "data" in data && "metadata" in data;
  }
